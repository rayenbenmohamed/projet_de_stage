...
const ProductDetail = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/products").then(res => {
      const proDetail = res.data.find((x) => x.id === parseInt(id, 10));
      setProduct(proDetail || {});
    });
  }, [id]);
      ...
      <div className="container my-5 py-3">
        <div className="row">
          ...
          <div className="col-md-6 d-flex flex-column justify-content-center">
            ...
            <button onClick={handleCart} className="btn btn-outline-primary my-5">{buttonText}</button>
          </div>
        </div>
      </div>
    </>
